...
export function WalletTroubleshootingGuide() {
  ...
  const faqItems = [
    ...
    {
      category: '🔧 セッション管理・長時間接続',
      items: [
        {
          question: 'PC環境で長時間接続していると「接続中」のままになる',
          answer: '💻 PC環境特有の問題です：\n1. MetaMaskセッションのタイムアウト\n2. 画面右下の接続状態インジケーターを確認\n3. 黄色い警告が表示された場合は「再接続」をクリック\n4. 自動で30秒ごとにセッション状態をチェックしています',
          isWarning: true
        },
        {
          question: 'セッションが無効になった場合の対処法',
          answer: '🔄 以下の手順で復旧してください：\n1. 表示される警告メッセージの「再接続」ボタンをクリック\n2. MetaMask拡張機能を一度無効化→有効化\n3. ブラウザを再起動\n4. 最後の手段：ページを更新（F5キー）'
        },
        {
          question: 'セッション監視機能について',
          answer: '🛡️ このアプリには以下の機能があります：\n• 30秒ごとの自動セッションチェック\n• ユーザーアクティビティ監視\n• MetaMaskアカウント変更の検出\n• 自動再接続機能\n• 接続状態の視覚的表示（右下インジケーター）'
        }
      ]
    },
    {
